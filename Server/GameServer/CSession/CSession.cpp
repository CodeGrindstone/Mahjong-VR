...
CSession::CSession(boost::asio::io_context& io_context, CServer* server):
	_socket(io_context), _server(server), _b_close(false),_b_head_parse(false), _user_uid(-1){
...
void CSession::Close() {
	...
	//此处将消息投递到逻辑队列中
	if(_user_uid != -1){
		_recv_msg_node->SetMsgId(MSG_QUIT);
		std::shared_ptr<LogicNode> logic_node = std::make_shared<LogicNode>(shared_from_this(), _recv_msg_node);
		LogicSystem::GetInstance()->PostToQue(logic_node);
	}
	_b_close = true;
}
...
LogicNode::LogicNode(std::shared_ptr<CSession> session,
                     std::shared_ptr<RecvNode> recvnode): m_session(session), m_recvnode(recvnode) {}
